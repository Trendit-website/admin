    ...
    <div className="flex flex-col gap-y-6 px-4 py-8 w-full text-[#667185]">
      ...
      <div className="flex items-center justify-between w-full px-4">
        <div>
          ...
          <span className="text-[14px]">Task ID: RET/15118/{userdetails?.user.id}</span>
        </div>
      ...
      <div className="w-full px-4 flex flex-col gap-y-2">
        <Image
          ...
          alt="profile picture"
        />
        <p className="text-primary-black text-[16px]">{userdetails?.user?.full_name}</p>
        <span className="text-secondary text-[14px]">USER ID: RET/15118/{userdetails?.user.id}</span>
